...
plot_knorm_VIS_inc = False #toggle normalized harmonic coefficients being plotted for increased velocities
plot_knorm_VIS_dec = False #toggle normalized harmonic coefficients being plotted for decreased velocities
...
if (plot_TISS or plot_knorm_all or plot_knorm_VIS_inc or plot_knorm_VIS_dec):
	...
	psr = SkyCoord(ra=str(psr_m.RAJ.quantity), dec=str(psr_m.DECJ.quantity), pm_ra_cosdec=psr_m.PMRA.quantity, pm_dec=psr_m.PMDEC.quantity, distance=fitval90['dpc'])
...
if (plot_knorm_VIS_inc):
	...
	fitval['VIS'] = -RotateVector(VIS,fitval['PsiAR']) #Rotate back into x and y
