...
# Import these from RickettEquations?  Change dpc to dpsr!!
fitval90 = {'i': 90. * u.deg,
            's': 0.71,
            'Oangle': 69 * u.deg,
            'R': 0.76,
...
if plot_knorm_all or plot_knorm_VIS_inc or plot_knorm_VIS_dec:
    ...
    k_fit = np.array(
        [[3.46, 3.79, 4.76, 6.19, 3.25, 0.96, 0.89, 0.92, 1.35,
         ...
         [-2.77, -3.51, -4.02, -4.40, -2.57, -1.07, -0.86, -0.84, -1.15,
          -1.13, -0.59, -0.62, -0.53, -0.67, -0.58, -0.49, -0.26]])

    # k_err[0] is K0 (col. 1 of Table 1), k_err[1] is Ks (col.2 of Table 1),
    # k_err[2] is Kc2 (col. 5 of Table 1)
    k_err = np.array([[0.06, 0.10, 0.13, 0.14, 0.08, 0.09, 0.05, 0.06,
                       0.06, 0.05, 0.05, 0.03, 0.04, 0.03, 0.02, 0.03, 0.01],
