...
def SystemVel(t_start, t_end, t_nsteps, fitval, psr):
    ...
    i = 0
    for v in VE:
        VC[i] = np.add(np.add(VP, v * u.km / u.s * fitval['s'] /
                              (1 - fitval['s'])), -fitval['VIS'] / (1 - fitval['s']))
        i += 1
...
def K_coeffs(t_start, fitval, psr, psr_m):
    ...
    return [K0 / (sp * sp), KS / (sp * sp), KC / (sp * sp),
            KS2 / (sp * sp), KC2 / (sp * sp)]
...
def k_norm(t_start, t_end, t_nsteps, fitval, psr, psr_m, bm):
    ...
    uy = np.sqrt(Qabc[1] / Qabc[0]) * (VC[:, 1] / V0 +
                                       psr_m.ECC.quantity * np.cos(i) * np.cos(bm.omega()))
    w = Qabc[2] / np.sqrt(Qabc[0] * Qabc[1])
    return [4 * ux + 2 * w * uy, -1 - 2 *
            ux * ux - 2 * w * ux * uy - 2 * uy * uy]
